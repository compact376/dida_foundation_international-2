...
export default function DonatePage() {
   ...
   const handleSubmit = async (e) => {
    ...
    if (!amount || isNaN(amount) || Number(amount) < 1) {
        setFormError('Please enter a valid donation amount (minimum $1).');
    ...
    try {
        ...
        const donationData = {
            ...
            donation_type: donationType.charAt(0).toUpperCase() + donationType.slice(1), // Capitalize first letter
            name: name.trim(),
            email: email.trim()
        };
        ...
        // Send single request to your Express backend
        ...
        if (response.ok && data.success && data.url) {
            // Show success UI
           // setRaisedAmount(prev => prev + parseFloat(amount || 0));
            setShowConfetti(true);
            setTimeout(() => setShowConfetti(false), 3000);
            
            // Redirect to Stripe after a brief delay to show confetti
            setTimeout(() => {
        ...
        setFormError(err.message || 'Network error. Please check your connection and try again.');
    } finally {
        setIsSubmitting(false);
    }
};
